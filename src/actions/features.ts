...
export async function updateLeaveStatusAction(requestId: string, newStatus: LeaveStatus, approverEmail: string) {
  try {
    ...
    // Retrieve the leave request to verify the applicant's role
    ...
    // If applicant is ORG_ADMIN, only SUPER_ADMIN can approve
    if (leaveRequest.employee.user?.role === Role.ORG_ADMIN && approver.user?.role !== Role.SUPER_ADMIN) {
      return { success: false, error: "Leave requests for ORG_ADMIN can only be approved/rejected by a SUPER_ADMIN." };
    }
...
export async function updatePayrollStatusAction(payrollId: string, newStatus: string, approverRole?: Role) {
  try {
    ...
    if (payroll.status === 'Pending Approval' && approverRole !== Role.SUPER_ADMIN) {
      return { success: false, error: "Only SUPER_ADMIN can approve payroll records generated by admins." };
    }
