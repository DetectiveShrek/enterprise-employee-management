...
export default function Dashboard() {
  ...
  const [selectedPayrollForStatus, setSelectedPayrollForStatus] = useState<any>(null);
  const [payrollStatusFormValue, setPayrollStatusFormValue] = useState<string>('');
  ...
  const handleCalculatePayroll = async (e: React.FormEvent) => {
    ...
    try {
      const res = await createPayrollAction({
        ...
        creatorRole: selectedRole as Role
      });
      if (res.success) {
  ...
  const handleTogglePayrollStatus = async (id: string, currentStatus: string) => {
    ...
    try {
      ...
      if (currentStatus === 'Draft') {
        ...
        if (selectedRole !== 'SUPER_ADMIN') {
          setRbacStatus({ type: 'error', text: 'Payrolls generated by admins must be approved by a SUPER_ADMIN.' });
          return;
        }
        nextStatus = 'Approved';
      ...
      const res = await updatePayrollStatusAction(id, nextStatus, selectedRole as Role);
      if (res.success) {
  ...
  const handleSavePayrollStatus = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedPayrollForStatus) return;
    setRbacStatus(null);

    if (payrollStatusFormValue === 'Approved' && selectedPayrollForStatus.status === 'Pending Approval' && selectedRole !== 'SUPER_ADMIN') {
      setRbacStatus({ type: 'error', text: 'Payrolls generated by admins must be approved by a SUPER_ADMIN.' });
    ...
    try {
      const res = await updatePayrollStatusAction(selectedPayrollForStatus.id, payrollStatusFormValue, selectedRole as Role);
      if (res.success) {
        setSelectedPayrollForStatus(null);
        setRbacStatus({ type: 'success', text: `Payroll status updated to ${payrollStatusFormValue}.` });
        fetchTabData();
      } else {
        setRbacStatus({ type: 'error', text: res.error || 'Failed to update payroll status.' });
      }
                    ...
                    <tr key={req.id} className="hover:bg-slate-50/40 transition-all">
                      ...
                      <td className="py-3.5 px-5 text-right space-x-1.5">
                        {req.status === 'PENDING' && selectedRole !== 'EMPLOYEE' && (
                          (() => {
                            const isApplicantOrgAdmin = req.employee?.user?.role === 'ORG_ADMIN';
                            const canApprove = !isApplicantOrgAdmin || selectedRole === 'SUPER_ADMIN';
                            ...
                            return (
                              <>
                                <button
                                  ...
                                  className="px-2 py-1 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-700 rounded-lg font-black text-[9px] uppercase border border-emerald-500/15 cursor-pointer"
                                >
                                  Approve
                                </button>
                                <button
                                  onClick={() => handleApproveLeave(req.id, false)}
                                  className="px-2 py-1 bg-red-500/10 hover:bg-red-500/20 text-red-755 rounded-lg font-black text-[9px] uppercase border border-red-500/15 cursor-pointer"
                                >
                                  Reject
                                </button>
                              </>
                    ...
                    <tr key={pr.id} className="hover:bg-slate-50/40 transition-all">
                      ...
                      <td className="py-3.5 px-5">
                        <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase border ${
                          ...
                          pr.status === 'Pending Approval' ? 'bg-amber-50 text-amber-700 border-amber-100' :
                      ...
                      <td className="py-3.5 px-5 text-right">
                        {isAccountsStaff ? (
                          <button
                            onClick={() => {
                              setSelectedPayrollForStatus(pr);
                              setPayrollStatusFormValue(pr.status);
                            }}
                            className="px-2 py-1 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg font-black text-[9px] uppercase border border-slate-200 transition-all cursor-pointer"
                          >
                            Change Status
                          </button>
                        ) : (
        ...
        {selectedPayrollForStatus && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs">
            <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-[350px] p-6 text-left">
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold text-slate-800 text-sm">Update Payroll Status</h3>
                <button 
                  onClick={() => setSelectedPayrollForStatus(null)} 
              ...
              <div className="mb-4 text-xs space-y-1.5 bg-slate-50 p-3 rounded-xl border border-slate-100">
                <p className="text-slate-500">Employee: <strong className="text-slate-700">{selectedPayrollForStatus.employee?.firstName} {selectedPayrollForStatus.employee?.lastName}</strong></p>
                <p className="text-slate-500">Period: <strong className="text-slate-700">{selectedPayrollForStatus.month}/{selectedPayrollForStatus.year}</strong></p>
                <p className="text-slate-500 flex items-center gap-1.5">
                  Current Status: 
                  <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase border ${
                    selectedPayrollForStatus.status === 'Paid' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                    selectedPayrollForStatus.status === 'Approved' ? 'bg-blue-50 text-blue-700 border-blue-100' :
                    selectedPayrollForStatus.status === 'Pending Approval' ? 'bg-amber-50 text-amber-700 border-amber-100' :
                    'bg-slate-50 text-slate-750 border-slate-100'
                  }`}>
                    {selectedPayrollForStatus.status}
                  </span>
                </p>
              </div>

              <form onSubmit={handleSavePayrollStatus} className="space-y-4 text-xs">
                <div>
                  <label className="block text-[10px] font-bold text-slate-455 uppercase mb-1">Select New Status</label>
                  <select
                    value={payrollStatusFormValue}
                    onChange={(e) => setPayrollStatusFormValue(e.target.value)}
                        ...
                        <tr key={u.id} className="hover:bg-slate-50/40 transition-all">
                          ...
                          <td className="py-3.5 px-5 text-right">
                            <div className="flex items-center justify-end gap-2">
                              {u.status === 'Pending Approval' && selectedRole === 'SUPER_ADMIN' && (
                                <button
                                  onClick={async () => {
                                    setRbacStatus(null);
                                    try {
                                      ...
                                      if (res.success) {
                                        setRbacStatus({ type: 'success', text: `User ${u.email} approved successfully.` });
                                        fetchTabData();
                                      } else {
                                        setRbacStatus({ type: 'error', text: res.error || 'Failed to approve user.' });
                                      }
                                    } catch (err) {
                                      setRbacStatus({ type: 'error', text: 'Error approving user.' });
                                    }
                                  }}
                                  className="px-2 py-1 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-700 rounded-lg font-black text-[9px] uppercase border border-emerald-500/15 cursor-pointer mr-1 animate-pulse"
                                >
                                  Approve User
                                </button>
                              )}
